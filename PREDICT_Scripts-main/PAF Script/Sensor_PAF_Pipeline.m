...
wpms.DATAIN     = [cwd 'PREDICT_EEG_Raw_Data' filesep]; % The raw data directory. All data is openly available on https://openneuro.org/datasets/ds005486/versions/1.0.0
wpms.DATAOUT    = [cwd 'Output' filesep]; % where the processed data gets saved 
wpms.FUNCTIONS    = [cwd 'eeglab_fieldtrip' filesep]; % EEGLAB AND FIELDTRIP should be kept in the "eeglab_fieldtrip" folder. 
% The current scripts call on "eeglab2019_1" and "fieldtrip-20200215", so if you have another version, you will need to update the script 
% on Lines 22-31 of Sensor_PAF_Pipeline.mat , Lines 23-32 of Manual_PAF_Pipeline.m and Lines 40-42 of Automated_PAF_Pipeline.m to the version that you are using.
%  e.g. if you are using "eeglab2023.0"
wpms.CHANNELS = [cwd 'channel_info' filesep] %% In "channel_info", we have already uploaded details of the 63 channels for the EEG recording (chanlocs.mat).

%Add functions
addpath([wpms.FUNCTIONS '\eeglab2019_1']) % add eeglab path
%The required EEGLAB plugins are also detailed here, including "bva-io1.5.13" and "firfilt".
...
addpath([wpms.FUNCTIONS '\fieldtrip-20200215']) %add fieldtrip path
addpath([wpms.FUNCTIONS '\fieldtrip-20200215\external\eeglab']) 
load([wpms.CHANNELS '\chanlocs.mat']) % add channel info file 
load([wpms.CHANNELS '\neighbour_template.mat']) %add neighbour tempalate file containing the neighbouring channels of each channel needed to interpolate any excluded electrodes
...
for px = 1:length(subjlist);
    ...
    all_eeg_files  = dir([wpms.DATAIN subjlist(px).name filesep 'eeg' filesep '*eeg*.vhdr']); %store all vhdr file directories 
    for day = 1:length(all_eeg_files);
        ...
        %lookup channels
        EEG = pop_chanedit(EEG, 'lookup', [wpms.FUNCTIONS 'eeglab2019_1\plugins\dipfit\standard_BESA\standard-10-5-cap385.elp']); %can also change this to chanlocs.mat
        
        EEG = pop_select(EEG, 'nochannel', {'GSR','HR','RESP'}); % remove auxilliary channels
        EEG = pop_reref(EEG, []); %average reference data
        EEG = pop_eegfiltnew(EEG, 2, 100, 826, 0, [], 1); %filter the data between 2 and 100Hz
        close all;
        ...
        figure; pop_spectopo(EEG, 1, [0 15000], 'EEG', 'freqrange', [2 100], 'electrodes','off'); %check that the frequency spectra is less noisy
...
for px = 1:length(subjlist)
    ...
    for day = 1:length(all_eeg_files);
        ...
        %Calculate PAF using a 8-12Hz window, with bins of .2 Hz
        paf = [];
        power = [];
        freq = 8:.2:12;
        for z = 1:size(data_freq.powspctrm,1);
            for y = 1:size(data_freq.powspctrm,2);
                ...
                temp2 = zscore(temp);
                paf(y,z) = sum(freq'.*(((temp(31:51)))))/sum(((((temp(31:51)))))); % Indices 31-51 refer to the 8-12 Hz window, while indices 36-46 refer to the 9-11 Hz window
