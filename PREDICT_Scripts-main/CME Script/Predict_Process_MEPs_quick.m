...
wpms.DATAIN     = [cwd 'PREDICT_TMS_Raw_Data' filesep];; % The data directory. All data is openly available on OSF https://osf.io/r3m9g/
wpms.DATAOUT    = [cwd 'Output' filesep]; % where the processed data gets saved 
wpms.GRIDLOC    = [cwd 'Gridloc' filesep]; %contains the Grid.mat file, consisting of the standard Grid Locations that were stimulated with the TMS coil (see folder for specific details).
...
for px = 1:length(subjlist) %Change the 1 here if you want start at a specific participant e.g. change 1 to 16
    ...
    spikefiles  = dir([wpms.DATAIN subjlist(px).name filesep 'tms' filesep subjlist(px).name '*-Map.mat']); %the .mat spike files from the mapping procedure
    ...
    for day = 1:length(spikefiles) %Change the 1 here if you want start at a specific session e.g. change 1 to 3 for the 3rd session
        ...
        fprintf(['\n Analysing session: ' exp.sessions{day} '\n\n']);
            ...
            stimuluated_locations    = table2array(loaded_coordinates(1:n_trials,2)); %store locations in a table 
            ...
            %% Map Parameters
            %active sites defined as a site greater than >10% of the maximum MEP amplitude   
